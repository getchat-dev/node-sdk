...
const flatten = (obj, prefix = '', posfix = '') => {
    const data = {};
...
const _trim = (value) => (_.isString(value) ? value.trim() : value);

const normalizeData = (data, filter = []) => {
    const result = {};
...
const normalizeChat = (chat) => normalizeData(chat, ['id', 'title', 'socket_port', 'create', 'type', 'metadata']);
...
const packObjectForSignature = (obj, key = '') => {
    const packed = [];
...
const addToSignature = (signature, data, filterKeys) => {
        ...
        keys.forEach((key) => {
            switch (_.getType(data[key])) {
                case _.TYPES.SCALAR:
                    ...
                    break;
                case _.TYPES.OBJECT:
                    packObjectForSignature(data[key], key).forEach((element) => {
