...
- Stop rules, in order: a page whose `pagination.current` isn't the page we asked for was clamped back to the last one, so it ends the walk **without being yielded** (`current: 0` means "nothing matched" and doesn't count); then an empty page (yielded only when it's the first, where its counts are the answer); then `next_page_url` (a URL continues, an explicit `null` stops even when the page count disagrees); if that key is absent, `pagination.total`; and failing that, whether the page came back full — judged by the server's own `items_per_page` when it reports one. The last two exist so an endpoint that reports nothing doesn't truncate silently; the clamp check is what keeps that fallback from looping forever, since the backend answers a too-far `page` with the last page again.
